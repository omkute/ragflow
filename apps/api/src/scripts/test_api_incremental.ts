...
function tokens(n:number) { return Array.from({length:n}, (_,i)=>`tok${i}`).join(' '); }
const chunkSize = 5;
const contentV1 = tokens(20*chunkSize);
console.log("POST /documents v1");
const res1 = await app.inject({ method: 'POST', url: '/documents', payload: { filename: `api-inc-${Date.now()}.txt`, content: contentV1 } });
console.log(`Status ${res1.statusCode}`, res1.json().id ? "id ok" : "no id");
console.log(res1.json());
if (![201,202].includes(res1.statusCode)) { console.error("FAIL post"); process.exit(1); }
const docId = res1.json().id;
const jobId1 = res1.json().jobId;
console.log(`Doc ${docId} job ${jobId1} version ${res1.json().currentVersion} status ${res1.json().status}`);
...
const contentV2 = tokens(20*chunkSize).replace('tok5','MODIFIED5').replace('tok25','MODIFIED25');
console.log("\nPOST /documents/:id/reindex");
const res2 = await app.inject({ method: 'POST', url: `/documents/${docId}/reindex`, payload: { content: contentV2 } });
console.log(`Status ${res2.statusCode}`, res2.json());
if (![201,202].includes(res2.statusCode)) { console.error("FAIL reindex"); process.exit(1); }
...
console.log(`Detail after reindex: currentVersion ${detail.json().currentVersion} status ${detail.json().status}`);
...
if (reused !== 18) { console.error("FAIL reused API"); process.exit(1); }
