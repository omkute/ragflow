...
const service = new DocumentService(db, { chunker, chunkerConfig: { chunkSize:5, chunkOverlap:0 }, embeddingProvider: fake, processInline: false });

function makeTokens(n: number) { return Array.from({length:n}, (_,i)=>`t${i}`).join(' '); }
...
if (chunksV2.length !== 10) { console.error("FAIL stale"); process.exit(1); }

// Old version's chunks still exist but not active
import { chunks } from '@indexa/db';
import { eq } from 'drizzle-orm';
const oldVersionChunks = await db.select().from(chunks).where(eq(chunks.documentVersionId, first.version!.id));
console.log(`Old version chunks still in DB: ${oldVersionChunks.length} (should 20, but not active)`);
if (oldVersionChunks.length !== 20) { console.error("FAIL old count"); process.exit(1); }
